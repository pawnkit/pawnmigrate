# Changelog

Notable changes are recorded here.

## 0.3.12 - 2026-08-04

### Changed

- Use the current pawn-analysis and pawn-api releases.

## 0.3.11 - 2026-08-03

### Changed

- Use pawn-analysis v0.30.22.

## 0.3.10 - 2026-08-03

### Changed

- Use pawnfmt v1.4.10 and pawn-parser v1.5.11.

## 0.3.9 - 2026-08-03

### Changed

- Use pawn-analysis v0.30.14, pawn-api v0.19.6, pawn-parser v1.5.10, and
  pawnfmt v1.4.9.

## 0.3.8 - 2026-08-02

### Changed

- Use the current analysis, API, parser, and formatter releases.

## 0.3.7 - 2026-08-02

### Changed

- Use pawn-analysis 0.30.11.

## 0.3.6 - 2026-08-02

### Changed

- Use pawn-analysis 0.30.10 for function-effect data.

## 0.3.5 - 2026-08-02

### Changed

- Use pawn-api 0.19.3.

## 0.3.4 - 2026-08-02

### Changed

- Use pawnfmt 1.4.7 and pawn-project 0.34.2.

## 0.3.3 - 2026-08-02

### Changed

- Use pawnfmt 1.4.6 and pawn-project 0.34.1.

## 0.3.2 - 2026-08-02

### Added

- Added a safe migration that removes duplicate `pawnkit.includePaths` entries.
- Manifest migrations can now compose their full-file edits in one plan.

### Changed

- Use pawn-analysis 0.30.9 and pawn-parser 1.5.8.

## 0.3.1 - 2026-08-01

### Changed

- Use pawn-parser v1.5.7.

## 0.3.0 - 2026-08-01

### Added

- Added the review-required `project.ysi-official` manifest migration.

## 0.2.4 - 2026-07-30

### Changed

- Updated project, parser, analysis, API, formatter, and core dependencies to
  the current PawnKit releases.

## 0.2.3 - 2026-07-30

### Changed

- Clarified that project and library-to-library migrations share the same
  preview and safety model.

## 0.2.2 - 2026-07-25

### Changed

- Added the repository support record with CI validation.

## 0.2.1 - 2026-07-24

### Changed

- Updated the analysis dependency to v0.1.15, which removes duplicate work
  and a quadratic scan from large-file analysis.

## 0.2.0 - 2026-07-23

### Changed

- Limited API rename migrations to reviewed, high-confidence metadata.

## 0.1.4 - 2026-07-23

### Changed

- Updated to the current formatter release.

## 0.1.3 - 2026-07-23

### Fixed

- Updated project discovery, analysis, and API metadata dependencies.

## 0.1.2 - 2026-07-23

### Fixed

- Updated API, project, analysis, and formatting dependencies.

## 0.1.1 - 2026-07-23

### Fixed

- Updated project discovery, parsing, analysis, and formatting dependencies.

## 0.1.0 - 2026-07-20

### Added

- Migration planning, previews, status reports, and atomic apply support.
- Built-in manifest, open.mp include, and deprecated-call migrations.
- Clean-worktree checks, stale-plan detection, and rollback.
- Human, JSON, and diff output.
- Release archives for Linux, macOS, and Windows.
